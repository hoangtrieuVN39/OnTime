...
FROM Employee
JOIN LeaveRequest ON Employee.EmployeeEmail = LeaveRequest.EmployeeEmail;

-- 2. Truy vấn để lấy thông tin về nhân viên và ca làm việc của họ
SELECT Employee.EmployeeName, WorkShift.ShiftName, Attendance.CreatedTime, Attendance.Statuss
FROM Employee
JOIN Attendance ON Employee.EmployeeID = Attendance.EmployeeID
JOIN WorkShift ON Attendance.ShiftID = WorkShift.ShiftID;

-- 3. Truy vấn để lấy thông tin về đơn từ và loại đơn từ
SELECT LeaveRequest.RequestID, LeaveRequest.CreatedTime, LeaveType.LeaveTypeName
FROM LeaveRequest
JOIN LeaveType ON LeaveRequest.LeaveTypeID = LeaveType.LeaveTypeID;

-- 4. Truy vấn để lấy thông tin về ca làm việc và số lượng nhân viên làm ca đó
SELECT WorkShift.ShiftName, COUNT(Attendance.AttendanceID) AS NumberOfEmployees
FROM WorkShift
LEFT JOIN Attendance ON WorkShift.ShiftID = Attendance.ShiftID
GROUP BY WorkShift.ShiftName;
